...
//----------------------------------
HRESULT XInput::initKeyboard(HWND& hwnd){

  HRESULT res;

	res=pInput->CreateDevice(GUID_SysKeyboard, &pKeyboard, NULL);
	if (FAILED(res)) return res;
     
	res=pKeyboard -> SetCooperativeLevel(hwnd, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE);
	if (FAILED(res)){
		pKeyboard->Release();
		pKeyboard=0;
		return res;
